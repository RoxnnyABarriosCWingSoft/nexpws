...
abstract class DomainPermissions
{
    ...
    public include<T>(permissions: (keyof T)[]): DomainPermissions
    {
    ...
    public exclude<T = Permissions>(permissions: (keyof T)[]): DomainPermissions
    {
        const _permissions = permissions.map(permission => (this as any)[permission]).filter(Boolean);
    ...
    public extends(...permissions: string[][]): DomainPermissions
    {
