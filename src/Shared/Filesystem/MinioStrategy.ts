...
class MinioStrategy implements IFilesystem
{
    ...
    async listObjects(payload: ListObjectsPayload)
    {
        ...
        const stream = this.#filesystem.listObjectsV2(this.getBucket(null, isPublic), prefix, recursive);
    ...
    {
        await this.#filesystem.removeObject(this.getBucket(object), `${this.#rootPath}/${object.name}`);
    }

    private getBucket(object?: IFileDomain, isPublic = false): string
    {
        const _isPublic = object?.isPublic ?? isPublic;
        return _isPublic ? this.#publicBucket : this.#privateBucket;
    }
