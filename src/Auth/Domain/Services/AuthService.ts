...
class AuthService
{
    ...
    {
        const rolePermissions = auth_user.roles.filter(role => role.enable).reduce((accum, role) =>
        {
    ...
    public getByRut(rut: string): Promise<Auth>
    {
        return this.userRepository.getOneByRut(rut);
    }

    public async authorize(authUser: Auth, handlerPermissions: string[], method: 'some' | 'every'): Promise<boolean>
    {
        ...
        return handlerPermissions[method]((hp: string) => totalPermissions.some((permission) => hp === permission));
    }
